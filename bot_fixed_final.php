...
$block = $from_id ? mysqli_query($connect, "SELECT * FROM `block` WHERE `id` = '" . mysqli_real_escape_string($connect, (string)$from_id) . "' LIMIT 1") : false;
...
function safe($text)
{
    ...
    $text = $connect->real_escape_string((string)$text);
    $array = ['$', ';', '"', "'", '<', '>'];
...
if (strpos($text, "/start ref_") === 0) {
    ...
    if (!empty($user['id']) or $ref_id == $from_id) {
        exit();
    }
