...
try {
    ...
    // Idempotent schema upgrades: only add legacy columns when they are missing.
    $ensureColumn = static function ($db, string $table, string $column, string $definition): void {
        ...
        $exists = ((int)$check->fetch_assoc()['c']) > 0;
        $check->free();

        if (!$exists) {
            $alter = "ALTER TABLE `{$table}` ADD COLUMN `{$column}` {$definition}";
            if (!$db->query($alter)) {
                throw new RuntimeException($db->error . ' | SQL: ' . $alter);
            }
        }
    };

    $ensureColumn($connect, 'user', 'daily_gift', 'BIGINT DEFAULT NULL');
    $ensureColumn($connect, 'settings', 'coin_daily', "INT DEFAULT '1'");
    $ensureColumn($connect, 'settings', 'coin_referral', "INT DEFAULT '1'");
    $ensureColumn($connect, 'settings', 'bot_mode', "VARCHAR(32) DEFAULT 'on'");
