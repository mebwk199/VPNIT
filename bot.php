...
date_default_timezone_set('Asia/Tehran');
$timestamp = time();
$ti = $timestamp; // Unix timestamp used by integer create_at/update_at columns.
$time = date('H:i');
$date = jalali_today();
$ToDay = jdate('l');
...
//==============================//  New user / Anti Spam //==============================//
// A webhook can be received from a brand-new Telegram user. In that case $user is
// null, so never read $user["spam"] before creating/loading the row.
if (!$user && $from_id !== null && $tc === 'private') {
    $safe_from_id = mysqli_real_escape_string($connect, (string)$from_id);

    $insert_user = $connect->query(
        "INSERT IGNORE INTO `user` (`id`, `join_date`, `create_at`, `update_at`)
         VALUES ('$safe_from_id', '$date', '$ti', '$ti')"
    );

    if (!$insert_user) {
        error_log('VPNIT: failed to create user ' . $from_id . ': ' . $connect->error);
    }

    // Reload the user so all later code works for a newly-created account.
    $user = mysqli_fetch_assoc(
        mysqli_query(
            $connect,
            "SELECT * FROM `user` WHERE `id` = '$safe_from_id' LIMIT 1"
        )
    ) ?: null;
}

// Anti-spam is only meaningful after the user row exists.
if ($user && isset($user["spam"]) && (int)$user["spam"] > time()) {
    exit();
}

if (!(is_bot_admin($from_id)) && $user) {
    // Store an integer Unix timestamp. The previous code used 0.2 seconds,
    // which gets truncated by an integer column.
    $tt = time() + 1;
    $connect->query("UPDATE `user` SET `spam` = '$tt' WHERE `id` = '$from_id' LIMIT 1");
}

//===========================// checking //===========================//
if ($block && mysqli_num_rows($block) > 0) {
    exit();
}
...
//===================// دریافت بکاپ دیتابیس //===================//
// Admin-only: creates a portable SQL dump of all tables and sends it as a Telegram document.
function createDatabaseBackup($connect)
{
    if (!$connect) {
        throw new RuntimeException('Database connection is not available.');
    }

    $filename = '/tmp/vpnit_database_backup_' . date('Ymd_His') . '.sql';
    $fh = @fopen($filename, 'wb');
    if (!$fh) {
        throw new RuntimeException('Could not create backup file.');
    }

    fwrite($fh, "-- VPNIT MySQL backup\n");
    fwrite($fh, "-- Generated: " . date('c') . "\n\n");
    fwrite($fh, "SET FOREIGN_KEY_CHECKS=0;\n");
    fwrite($fh, "SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO';\n\n");

    $tablesResult = mysqli_query($connect, "SHOW TABLES");
    if (!$tablesResult) {
        fclose($fh);
        @unlink($filename);
    ...
    while ($tableRow = mysqli_fetch_row($tablesResult)) {
        ...
        $createResult = mysqli_query($connect, "SHOW CREATE TABLE $tableEscaped");
        if (!$createResult) {
            fclose($fh);
            @unlink($filename);
            throw new RuntimeException('Could not read structure of table: ' . $table);
        }
        ...
        $dataResult = mysqli_query($connect, "SELECT * FROM $tableEscaped");
        if (!$dataResult) {
            fclose($fh);
            @unlink($filename);
        ...
        while ($row = mysqli_fetch_row($dataResult)) {
            ...
            for ($i = 0; $i < $fieldCount; $i++) {
                if ($row[$i] === null) {
                    ...
                    $values[] = "'" . mysqli_real_escape_string($connect, (string)$row[$i]) . "'";
                }
            }
            fwrite($fh, "INSERT INTO $tableEscaped VALUES (" . implode(',', $values) . ");\n");
        }
...
if ($text == '💾 دریافت بکاپ دیتابیس' && $tc == 'private' && is_bot_admin($from_id)) {
    bot('sendmessage', [
        'chat_id' => $chat_id,
        'text' => "⏳ در حال ساخت بکاپ کامل دیتابیس...\nلطفاً چند لحظه صبر کنید.",
    ]);

    $backupFile = null;
    try {
        $backupFile = createDatabaseBackup($connect);
        $sizeMb = round(filesize($backupFile) / 1048576, 2);

        $result = bot('sendDocument', [
            'chat_id' => $chat_id,
            'document' => new CURLFile($backupFile, 'application/sql', basename($backupFile)),
            'caption' => "✅ بکاپ کامل دیتابیس VPNIT آماده شد.\n📦 حجم: {$sizeMb} MB\n🔐 این فایل را در جای امن نگه دارید.",
        ]);

        if (!$result || empty($result->ok)) {
            bot('sendmessage', [
                'chat_id' => $chat_id,
                'text' => "❌ ارسال فایل بکاپ به تلگرام ناموفق بود.\nاگر حجم فایل زیاد است، حجم فایل را بررسی کنید.",
            ]);
        }
    } catch (Throwable $e) {
        ...
        bot('sendmessage', [
            'chat_id' => $chat_id,
            'text' => "❌ ساخت بکاپ ناموفق بود. خطا در لاگ Railway ثبت شد.",
        ]);
    } finally {
